...
import { Play, BarChart2, CheckCircle2, Star } from 'lucide-react';
...
export const LibraryView: React.FC<LibraryViewProps> = ({ tracks, currentTrack, isPlaying, onPlay }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full pb-32 overflow-y-auto pt-4 px-4">
      <h1 className="text-3xl font-bold mb-6 text-slate-900 dark:text-white tracking-tight">Library</h1>
      ...
      <div className="space-y-4">
        {tracks.map((track) => {
          ...
          return (
            ...
            >
              <div className="relative w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 shadow-md dark:shadow-lg">
                <img 
                  src={track.coverUrl} 
                  alt={track.title} 
              ...
              <div className="ml-4 flex-1 min-w-0">
                <div className="flex items-center space-x-2">
                  <h3 className={`font-semibold truncate text-lg ${isCurrent ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-900 dark:text-white'}`}>
                    {track.title}
                  </h3>
                  {track.downloaded && (
