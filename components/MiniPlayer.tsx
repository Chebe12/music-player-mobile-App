...
export const MiniPlayer: React.FC<MiniPlayerProps> = ({ playbackState, controls, onExpand }) => {
  ...
  return (
    <div 
      className="fixed bottom-[4.5rem] left-2 right-2 md:left-4 md:right-4 bg-slate-800/90 backdrop-blur-lg rounded-2xl p-2 shadow-2xl z-40 cursor-pointer border border-white/5 transition-transform active:scale-[0.99]"
      onClick={onExpand}
    >
      {/* Progress Line */}
      ...
      <div className="flex items-center pt-1.5 px-1">
        {/* Art */}
        <div className={`w-10 h-10 rounded-full overflow-hidden flex-shrink-0 animate-[spin_10s_linear_infinite] ${!isPlaying ? 'paused' : ''}`}>
        ...
        <div className="ml-3 flex-1 min-w-0 flex flex-col justify-center">
          ...
          <p className="text-xs text-indigo-300 truncate leading-tight">{currentTrack.artist}</p>
        </div>
